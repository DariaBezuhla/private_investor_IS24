...
import 'dart:async';
import 'dart:convert';
import 'package:privateinvestorsmobile/locations/location.dart';
...
class AutoCompleteLocationService {
  ...
  Future<List<Location>> fetchAutocompleteLocation ({String location = 'berlin'}) async {
    ...
    if (response.statusCode == 200) {
      var cities = json.decode(response.body)['city']; // possibly list
      var postcodes = json.decode(response.body)['postcode']; // List
      var quarterOrTowns = json.decode(response.body)['quarterOrTown']; // definietely list
      var countries = json.decode(response.body)['country'];
